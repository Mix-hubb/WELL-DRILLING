...
import { buildLineNoticeFlex, sendTextToCustomerById } from "../services/line";
...
async function handleText(userId: string, text: string, replyToken: string, org: OrgLineConfig, baseUrl: string = "") {
  ...
  const frontendUrl = process.env.FRONTEND_URL || process.env.CORS_ORIGIN || "https://well-drilling.vercel.app";

  if (/แจ้งเจาะ|ขุดเจาะ|เจาะบ่อ/.test(text)) {
    const liffId = org.line_liff_id_drilling;
    const liffUrl = liffId
      ? `https://liff.line.me/${liffId}`
      : `${frontendUrl}/request-drill`;
